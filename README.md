...
**Tila:** **v1.0.2** tutkimuspaketti, julkaistu 2026-08-20. Katso `CHANGELOG.md` mitä on muuttunut v1.0.0:sta lähtien, ja repositorion [Julkaisut-sivu](https://github.com/valto/tekoalytyokapasiteetin-vallankumous/releases) viitattavia, tarkistussummalla vahvistettuja tilannekuvia joka julkaisusta varten.
...
**Avoin ilmoitus:** kirjoittajalla on kommersiaalisia ja edunajointiin liittyviä intresseja henkilökohtaiseen tekoälyinfrastruktuuriin ja osuustoiminnallisiin/omistusperustaisiin tekoälymalleihin Prifinan, Digiolen, ValtoAI:n ja PIOS- ja EIOS-viitekehyksien kautta. Katso alkutekstin (`01-whitepaper.md`) Menetelmät §3.4 täydelle avoimelle ilmoitukselle ja ei-neuvonta-rajalle.

## Lukupolut — aloita tästä, ei koko paketista kerralla

Tämä paketti on rakennettu niin, että eri lukijat voivat saada yhteisen faktapohjan eri syvyyksillä, joutumatta kaikki läpikäymään samaa pitkää asiakirjaa ensin:
...
| `assets/diagrams/diagram-01…11-*.png` | 11 valmista kaaviokuvaa, jotka on määritelty `17-visual-asset-briefs.md`:ssä — Kaavio 11 on kirjoittajan oma ennalta olemassa oleva kehyskaavio, provenienssikehystetty (näyttöluokkatunniste, viittaus, alatunniste) vastaamaan muuta joukkoa; muokkaamaton alkuperäinen säilytetään sen vierellä tiedostona `diagram-11-*.jpg` | Raportti-/diaesitys-/verkkokäyttö |
...
Joka `*.html`-tiedosto repositorion juuressa paitsi `index.html` ja `diagrams.html` (käsin kirjoitetut) ja `perspectives.html` (generoitu erillisellä skriptillä, alla) generoidaan vastaavasta `.md`-tiedostostaan — **ei käsin muokata generoitua `.html`-tiedostoa**; muokkaa markdown-lähdettä ja rakenna uudelleen:
...
- **Ratkaistu (2026-08-14):** tämä paketti on nyt julkaistu GitHub Pages -sivustona osoitteessa [valto.github.io/ai-working-capacity-revolution](https://valto.github.io/ai-working-capacity-revolution/) (englanninkielinen alkuperäinen), seuraten `main`:ia — katso yllä oleva Tila-rivi siitä, miten elävä sivusto liittyy viimeksi tagattuun julkaisuun. Tämä suomenkielinen käännös on julkaistu vastaavasti omana Pages-sivustonaan osoitteessa [valto.github.io/tekoalytyokapasiteetin-vallankumous](https://valto.github.io/tekoalytyokapasiteetin-vallankumous/).
- **Ratkaistu (2026-08-20):** version/tagin/julkaisun ristiriita korjattu — repositoriolla oli aiemmin vain `v1.0.0`-tagi ilman GitHub-julkaisua, vaikka README ja `CITATION.cff` väittivät `v1.0.1`:tä. Koska sisältö sisälsi jo kaikki `v1.0.2`-tavoitteelliset muutokset (elävän viitekehyksen §3.6, korjatut työnantaja-/laskutettava-luvut, koko Pages-sivustoarkkitehtuurin), versio nostettiin suoraan `v1.0.2`:een kaikessa metadatassa (README, `CITATION.cff`, `index.html`), tagattiin ja julkaistiin GitHub-julkaisuna tarkistussummalla vahvistetun tilannekuvan kanssa. Samassa kierroksessa tehtiin natiivi suomen kielenhuollon korjauspassi — katso yllä oleva Muutosloki-viittaus.
